...
fn main() {
    ...
    println!("2. Vault indexed. Forward Links: {}", vault.graph.forward_links.len());
    
    let vault_arc = Arc::new(RwLock::new(vault));

    // 2. Start watcher
    println!("3. Starting watcher...");
    let _watcher = VaultWatcher::watch(&test_dir, Arc::clone(&vault_arc)).expect("Failed to start watcher");
    ...
    fs::write(&file_path, "Modified content with [[Link B]] and [[Link C]]").unwrap();
