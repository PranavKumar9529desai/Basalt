...
export type CanvasXYNode = Node<Record<string, any>, "canvasText" | "canvasFile" | "canvasLink" | "canvasGroup">;

export function mapToXYFlow(doc: CanvasDocument): { nodes: CanvasXYNode[]; edges: Edge[] } {
    ...
    switch (node.type) {
      ...
      case "group":
        return {
          ...baseNode,
          type: "canvasGroup",
          data: { label: node.label, background: node.background, backgroundStyle: node.backgroundStyle, color },
        } as CanvasXYNode;
    }
  });
...
export function mapToCanvasDocument(nodes: CanvasXYNode[], edges: Edge[]): CanvasDocument {
  ...
  const canvasNodes: CanvasNode[] = realNodes.map((n) => {
    ...
    switch (n.type) {
      ...
      case "canvasFile":
        return { ...base, type: "file", file: (data.file as string) || "", subpath: data.subpath as string | undefined };
      case "canvasLink":
