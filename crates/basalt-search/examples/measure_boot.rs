...
fn main() {
    ...
    println!("  1. fast_scan_flat_tree:   {:>6.2} ms  ({} entries)", tree_time.as_secs_f64() * 1000.0, flat_tree.len());
    ...
    println!("  -> Tier 1 Cold Boot Total:{:>6.2} ms  (Window paints immediately!)", cold_tier1_total.as_secs_f64() * 1000.0);
    ...
    println!("  Full vault parse (Rayon): {:>6.2} ms  ({note_count} notes, {:.0} notes/sec)", index_time.as_secs_f64() * 1000.0, throughput);
    ...
    println!("  1. VaultCache::load:      {:>6.2} ms  ({} notes in cache)", cache_load_time.as_secs_f64() * 1000.0, loaded_cache.vault.note_count());

    let t = Instant::now();
    let _ = fast_scan_flat_tree(p);
    let tree_time_warm = t.elapsed();
    println!("  2. fast_scan_flat_tree:   {:>6.2} ms", tree_time_warm.as_secs_f64() * 1000.0);

    let t = Instant::now();
    let _ = SearchState::open_fast(&index_dir, paths.clone()).expect("search fast open");
    let search_time_warm = t.elapsed();
    println!("  3. search open_fast:      {:>6.2} ms", search_time_warm.as_secs_f64() * 1000.0);

    let warm_tier1_total = cache_load_time + tree_time_warm + search_time_warm;
    println!("  -> Tier 1 Warm Boot Total:{:>6.2} ms  (Instant boot!)", warm_tier1_total.as_secs_f64() * 1000.0);
