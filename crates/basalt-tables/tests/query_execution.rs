...
fn vault_with_docs() -> Vault {
    ...
    // Note: add_document populates arena + graph metadata cache.
...
fn table_query_returns_all_pages_and_columns() {
    ...
    assert_total(&result, 2); // alpha + gamma
    assert_eq!(result.rows.len(), 2);
    // First column is the selected file.name field.
...
fn limit_clause_truncates_rows_but_keeps_total() {
    ...
    assert_total(&result, 3); // total reflects pre-limit count
