...
fn main() {
    let input = fs::read_to_string(
        "/home/pranav/Projects/Basalt/crates/basalt_core/examples/edge_cases_test.md",
    )
    .unwrap();
    let doc = TextDocument::new(&input);

    let output_filepath = "/home/pranav/Projects/Basalt/scratch/edge_cases_output.txt";
    ...
    report.push_str(
        "Mapping visualizer (Showing Char | UTF-8 Byte Offset | UTF-16 Code Unit Offset):\n",
    );
    report.push_str("--------------------------------------------------------------------------\n");
    ...
    if let Some(rocket_idx) = input.find("🚀") {
        ...
        report.push_str(&format!(
            "Rocket Emoji 🚀 found at Byte Offset: {}\n",
            rocket_idx
        ));
        report.push_str(&format!(
            "Mapped to CodeMirror UTF-16 Offset: {}\n",
            utf16_rocket
        ));
        report.push_str(&format!(
            "Mapped back to Rust Byte Offset: {}\n",
            mapped_back_byte
        ));
