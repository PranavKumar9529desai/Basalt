...
```
EditorArea
└── SplitNode (orientation: horizontal = side-by-side columns)
    ├── LeafNode (tab group A)
    ├── SplitNode (orientation: vertical = stacked rows)
...
→ `"vertical"` (rows) — matching `edgeToSplit` (left/right edges →
`"horizontal"`, top/bottom → `"vertical"`).
