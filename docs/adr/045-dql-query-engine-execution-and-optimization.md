...
| **25k Notes Scan & Filter**  | $\approx 250 - 600\text{ms}$ _(Blocks UI)_ | $\approx 35\text{ms}$ (Full clone) | **$\le 5\text{ms}$** _(Predicate push-down)_             |
...
- **Solution**:
  ...
  - In `build_page_rows`, extract and clone _only_ the specific fields requested. Unreferenced large frontmatter structures (such as long YAML summaries or nested arrays) are completely ignored.
...
- **Predictable Memory Footprint**: Push-down filtering and top-k selection keep RAM usage proportional to the _result set_ rather than the entire vault size.
