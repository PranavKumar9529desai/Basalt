...
| **100 KB Keystroke (p95)** | $\approx 15\text{ms} - 35\text{ms}$ _(Frame drops, typing stutter)_ | $\approx 3.1\text{ms} - 4.0\text{ms}$ _(Passed ADR-019 gate)_          | $\mathbf{\le 2.0\text{ms}}$ _(Rock-solid sub-2ms floor)_   |
